 ...
 *
 * Copyright (C) 2010 NeoCat (neocat@neocat.jp)
 *
...
static void yurex_delete(struct kref *kref)
{
	...
	if (dev->urb) {
		...
		if (dev->int_buffer)
			usb_buffer_free(dev->udev, YUREX_BUF_SIZE,
...
static int yurex_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	dev->cntl_req = usb_buffer_alloc(dev->udev, YUREX_BUF_SIZE,
					 GFP_KERNEL,
	...
	dev->cntl_buffer = usb_buffer_alloc(dev->udev, YUREX_BUF_SIZE,
					    GFP_KERNEL,
	...
	dev->cntl_urb->transfer_flags	|= URB_NO_SETUP_DMA_MAP |
	...
	dev->int_buffer = usb_buffer_alloc(dev->udev, YUREX_BUF_SIZE,
